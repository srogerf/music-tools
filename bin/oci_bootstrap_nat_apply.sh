...
usage() {
  cat >&2 <<'EOF'
Usage: bash bin/oci_bootstrap_nat_apply.sh [--var-file FILE] [--state-file FILE] [--plan-file FILE] [terraform apply args...]
...
reject_placeholder_tfvars_values "$TFVARS_PATH" "Fill in real OCI values, then rerun plan before applying." compartment_ocid vcn_id private_subnet_id
...
if [[ ! -f "$PLAN_PATH" ]]; then
  ...
  echo "Run bash bin/oci_bootstrap_nat_plan.sh before applying." >&2
  exit 1
fi
