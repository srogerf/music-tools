...
usage() {
  cat >&2 <<'EOF'
Usage: bash bin/oci_terraform_apply.sh [--var-file FILE] [terraform apply args...]
...
Examples:
  ...
  bash bin/oci_terraform_apply.sh --var-file terraform.tfvars.staging
  bash bin/oci_terraform_apply.sh -auto-approve
EOF
}
...
terraform -chdir="$OCI_DIR" apply -var-file="$TFVARS_FILE" "${extra_args[@]}"
