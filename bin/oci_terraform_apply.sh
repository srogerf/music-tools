...
usage() {
  cat >&2 <<'EOF'
Usage: bash bin/oci_terraform_apply.sh [--var-file FILE] [--backend-config FILE] [--plan-file FILE] [terraform apply args...]
...
Examples:
  ...
  bash bin/oci_terraform_apply.sh --backend-config .private/oci/app.test.backend.hcl
